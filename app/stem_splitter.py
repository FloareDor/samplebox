...
def extract_drum_hits(drum_stem_path, output_path, amplitude_threshold=0.1, max_samples_per_category=5):
    ...
    for i, start in enumerate(onset_samples):
        ...
        # Store hit along with its low frequency amplitude for kicks, regular amplitude for others
        if drum_type == "kick":
...
def extract_melodic_samples(other_stem_path, output_path, amplitude_threshold=-100, max_samples=18, creative_mode=True):
    y, sr = librosa.load(other_stem_path)
    ...
    for i, (sample, start_time, _) in enumerate(samples):
        ...
        filename = f"{str(key).replace("#","-sharp")}.wav"
        sf.write(os.path.join(output_path, filename), sample, sr)
   
    print(f"Extracted {len(samples)} one-bar melodic samples to {output_path}")


def create_drum_loops_from_original(drums_file, output_dir, num_loops=5, bars_per_loop=2):
    # Load the drum stem
    y, sr = librosa.load(drums_file)
...
if __name__ == "__main__":
    ...
    drum_loops_dir = os.path.join(results_dir, "drum_loops")
    os.makedirs(drum_loops_dir, exist_ok=True)

    drums_file = os.path.join(results_dir, "drums.wav")
    # create_drum_loops_from_original(drums_file, drum_loops_dir)
    
    # Extract drum hits
    drum_dir = os.path.join(results_dir, "drum_hits")
    os.makedirs(drum_dir, exist_ok=True)
    # extract_drum_hits(os.path.join(results_dir, "drums.wav"), drum_dir, amplitude_threshold=0.1)
   
    # Extract melodic samples
    melodic_dir = os.path.join(results_dir, "melodic_samples")
    os.makedirs(melodic_dir, exist_ok=True)
    # extract_melodic_samples(os.path.join(results_dir, "other.wav"), melodic_dir)
    
    melody_sample = os.path.join(results_dir, "other.wav")
    instrumental_wonky_output_folder = os.path.join(results_dir, "wonky_samples_instrumental")
